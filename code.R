...
Total_Data_subset_precovid <- dplyr::filter(Total_Data_subset, COVID==0 & Season.x != "2019-20")# create subset precovid exclude 2019-20 since half was played with fans and half withou
Total_Data_subset_postvar <- dplyr::filter(Total_Data_subset, VAR==1 & Season.x != "2019-20" & Season.x != "2020-21") #create subset postvar and precovid for robustness checks

### histograms

#histogram of skin tone
plot0 <- ggplot(data=Total_Data_subset, aes(x=Skin )) +
  geom_bar() +
  labs (x="Skin Tone", y = "Count") +
...
#create combined plot of fouls and cards for manuscript
...
## combined plots of predicted values for fouls and cards

top_row = ggarrange(m1, m2, ncol = 2, labels = c("a)", "b)"))
bottom_row = ggarrange(NULL, m3, NULL, ncol = 3, labels = c("", "c)", ""), widths = c(1,2,1))
final_plot = ggarrange(top_row, bottom_row, ncol = 1)

# calculate differences between dark-skinned and light-skinned players (85th vs 15th percentiles) for fouls and cards
# to convey substantial importance of findings
...
# combine pre and post COVID differences 
comp_fouls <- rbind.fill(comp_pre, comp_post)

#plot with 95%CI only
